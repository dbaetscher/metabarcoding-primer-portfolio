...
loc_accum_prop_combo_minus_3 <- loc_accum_prop_combo %>%
...
ggsave("pdf_outputs/ref_spp_FRP_w_unique_taxa_per_locus_May2021.pdf", width = 7, height = 5)
```
...
# list of reference species, the locus that identified that species, and the barcoding gene
spp_locus_gene_df <- spp_locus_gene_df %>%
  select(-gene)

# now take the reference data and join the locus/spp info
spp_locus_gene_df %>%
  full_join(., ref_db_long) %>%
  group_by(locus, barcode) %>%
  select(species, locus, barcode, db_presence) %>%
  mutate(spp_not_identified = ifelse(db_presence == 1 & is.na(locus), 1, NA))
...
# reference_spp %>%
#   pivot_longer(cols = 5:9, names_to = "gene", values_to = "blast_entries") %>%
#   mutate(db_presence = ifelse(blast_entries > 0, 1, 0)) %>% # make this a binary variable
#   select(species, gene, db_presence) %>%    
#   unique() %>%
#   left_join(., spp_df)
...
```{r which-spp-missing-from-db}
reference_spp %>%
  filter(str_detect(species, "_sp"))

reference_spp %>%
  anti_join(spp_locus_gene_df) %>%
  select(species) %>%
  write.csv("~/Desktop/no-spp-ref.csv")
