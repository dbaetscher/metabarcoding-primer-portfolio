...
Partway through cleaning this up: 28 Oct 2020

The goal here is to read in the data output from `03-species-occupancy-model.Rmd` and filter the existing data set to remove those ASVs that had <80% probabily.
...
ls -l *VRP_ASV_SODM_100kiter.csv | awk 'BEGIN {print "locus", "file"} NR >1 && !/posinfo/ {num = $NF; gsub(/filename/, "", num); gsub(/[._].*$/, "", num);  print num, $NF}' > ../../../data/VRP-SODM-file-list.txt 
```

```{r create-dataframe-from-csvs}
# get the names of the files
fdf <- read.table("~/Documents/git-repos/metabarcoding-primer-portfolio/data/VRP-SODM-file-list.txt", stringsAsFactors = FALSE, header = TRUE) %>%
  tbl_df()
dir <- "~/Documents/git-repos/metabarcoding-primer-portfolio/Rmd/csv_outputs/sodm"
...
```
111 ASVs that are removed at the 80% filtering threshold.
...
# save that
saveRDS(filtered80perc_df_spp_98, "../data/voucher_features_sodm_filtered_taxonomy_df.rds", compress = "xz")
...
```{sh, eval=FALSE}
ls -l *FRP_ASV_SODM_100kiter.csv | awk 'BEGIN {print "locus", "file"} NR >1 && !/posinfo/ {num = $NF; gsub(/filename/, "", num); gsub(/[._].*$/, "", num);  print num, $NF}' > ../../../data/FRP-SODM-file-list.txt 

```

```{r create-frp-dataframe-from-csvs}
# get the names of the files
fdf <- read.table("~/Documents/git-repos/metabarcoding-primer-portfolio/data/FRP-SODM-file-list.txt", stringsAsFactors = FALSE, header = TRUE) %>%
  tbl_df()
dir <- "~/Documents/git-repos/metabarcoding-primer-portfolio/Rmd/csv_outputs/sodm"
...
# save output
# full reference
saveRDS(FRP_filtered80perc_df, "../data/full_reference_sodm_filtered_taxonomy_df.rds", compress = "xz")
```
