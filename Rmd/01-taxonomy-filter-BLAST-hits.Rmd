...
# taxonomy df"
taxa_dataframe <- readRDS("../data/taxonomy_ASV_BLAST_hits_fixed_accession_df.rds")
...
saveRDS(uncollapsed_taxonomy_spp_98, "../data/uncollapsed_taxonomy_spp_98.rds", compress = "xz")
# unique - only a single BLAST hit per ASV... this opts for the taxonomic level that corresponds to the filtering rules above
saveRDS(unique_taxonomy_spp_98, "../data/unique_taxonomy_spp_98.rds", compress = "xz")

```

## tracking down the 18 ASVs that are missing from the final taxonomy df

```{r 18-missing-ASVs-to-find}
### hunting down the discrepancies:
test2 <- clean_taxonomy_df_unique %>%
  select(locus, seq)

test1 <- taxa_dataframe %>%
  select(locus, seq) %>%
  distinct()
...
# missing ASVs present in taxonomy df
missing_asvs %>%
  left_join(., taxonomy_fixed_unique, by = c("locus", "seq"))
```

```{r fixing-the-missing-ones}
# if I can't figure out exactly where/why they went missing, at least we can restore them here
missing_ranked_hits <- missing_asvs %>%
  left_join(., taxonomy_fixed_unique, by = c("locus", "seq")) %>%
  group_by(locus, seq, species, genus, family, domain) %>% # fewer errors in these levels
  mutate(taxon_percID = max(perc_id)) %>%
  group_by(locus, seq, species, genus, family, domain, taxon_percID) %>%
...
# here's the df of the sequences with a single top hit:
missing_top_hits <- missing_ranked_hits %>%
  group_by(locus, seq, id_rank) %>%
  filter(id_rank == 1) %>% # tally the number of entries for a given sequence ranked 1
  tally() %>%
  filter(n == 1) %>% # keep only sequences that have a single top-ranked entry
...
# only those single-top-hit sequences are eligible for testing whether there is a > 2% difference
missing_top_hits_2perc <- missing_top_hits %>%
  left_join(., missing_ranked_hits, by = c("locus", "seq")) %>%
  arrange(locus, seq, id_rank) %>%
  filter(id_rank == 2) %>% # I'm only interested in the second-top-hit at this stage. I should have removed all of the multi-top-ranked sequences, which means, all of these sequences have an entry for id_rank = 2 
  filter(diff > 2) %>% # only the hits that have > 2% difference between the first and second hits
  select(locus, seq) %>% 
...
# none of the ASV sequences have a single top hit with < 2% difference between hits 1 and 2
missing_top_hits %>%
  anti_join(., missing_top_hits_2perc)

# so these are the missing ASV assignments, all of which have a genus-level assignment.
missing_asvs_fixed <- missing_top_hits_2perc %>%
  mutate(taxonomic_level ="genus") %>%
  mutate(taxon = genus) %>%
  select(-id_rank, -top_perc, -diff)
```

```{r combine-with-final-taxonomy-df}
merged_missing_w_taxonomy_df_spp_98 <- uncollapsed_taxonomy_spp_98 %>%
  bind_rows(missing_asvs_fixed) 

merged_missing_w_taxonomy_df_spp_98 %>%
  select(locus, seq) %>%
  unique()
```

Now summarize those data!

```{r}
# for the consensus taxonomy:
merged_unique_taxonomy_spp_98 <- merged_missing_w_taxonomy_df_spp_98 %>%
  group_by(locus, seq)%>%
  arrange(locus, seq, -taxon_percID) %>%
  mutate(top_perc_id = max(taxon_percID)) %>%
  select(locus, seq, taxon, taxonomic_level, top_perc_id) %>%
...
## modify the dataset with these additional 18 ASVs?!